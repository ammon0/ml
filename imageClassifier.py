...
def imageGender(profileTable, modulePath, imageDir):

    weightsDir=modulePath+'/images/training/weights/'
    genderClassifier=load_model(weightsDir+'gender-weights-1292-0.88.h5')
    result = pd.DataFrame(index=profileTable['userid'], columns=['gender'])
    count=0
    for i in tqdm(range(profileTable.shape[0])):
        thisUser=profileTable['userid'][i] 
        #try:
            #img = image.load_img(imageDir + thisUser + '.jpg', target_size=(64,64,3))
        #    count+=1
        #except:
        ...
        if ((detectFace(imageDir + thisUser + '.jpg') != 1)):
            ### use BASELINE
            #print('using baseline')
            #unused+=1
            result.at[thisUser, 'gender'] = np.ones(1)
